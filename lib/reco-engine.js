import { getEmbeddings, getManifest, getMask } from "./data.js";
import { cosineSimilarity, weightedAverageVectors, weightedCosineSimilarity } from "./vector-math.js";
...
export async function getRecommendations(userEvents, options = {}) {
  ...
  const useLatentMask = options.useLatentMask === true;
  const mask = useLatentMask ? await getMask(category) : null;
  const maskWeights = Array.isArray(mask?.weights) ? mask.weights : null;
  ...
  const scoredProducts = manifest.map(product => {
    ...
    const score = productVector
      ? (maskWeights ? weightedCosineSimilarity(userVector, productVector, maskWeights) : cosineSimilarity(userVector, productVector))
      : 0;
